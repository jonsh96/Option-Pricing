function [times, prices, variances, sample_sizes] = MonteCarlo(K, Smin, Smax, rate, volatility, dt, T, M, option_payoff, option_price, barrier)
    ...
    if(size(M,2) == 1)
        ...
        % Naive method
        [times(1), prices(1,:), variances(1,:), sample_sizes(1,:)] = NaiveMethod(K, Smin, Smax, rate, volatility, dt, T, M(1), option_payoff, barrier);
        ...
        % Importance sampling
        [times(4), prices(4,:), variances(4,:), sample_sizes(4,:)] = ImportanceSampling(K, Smin, Smax, rate, volatility, dt, T, M(4), option_payoff);
    end
end